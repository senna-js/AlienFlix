...
export const getSourceUrl = (
  ...
  const urlTemplate =
    mediaType === "movie" ? source.urls.movie : source.urls.tv;
...
export const getDefaultSource = (): VideoSource => {
  // Get from localStorage or use first non-French source as default
  const savedSourceId = localStorage.getItem("preferredVideoSource");
  if (savedSourceId) {
    const savedSource = availableSources.find(
      (source) => source.id === savedSourceId,
    );
    if (savedSource) return savedSource;
  }

  return (
    availableSources.find((source) => !source.isFrench) || availableSources[0]
  );
};

export const savePreferredSource = (sourceId: string): void => {
  localStorage.setItem("preferredVideoSource", sourceId);
};
